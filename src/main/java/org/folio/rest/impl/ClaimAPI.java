...
public class ClaimAPI implements Claim {
  ...
  public ClaimAPI(Vertx vertx, String tenantId) {
            ...
            org.folio.rest.jaxrs.model.Claim.class, fieldList, cql, true, false, reply -> {
              try {
                if (reply.succeeded()) {
                  ...
                  asyncResultHandler.handle(io.vertx.core.Future.succeededFuture(Claim.GetClaimResponse
  ...
  public void postClaim(String lang, org.folio.rest.jaxrs.model.Claim entity, Map<String, String> okapiHeaders,
    ...
    vertxContext.runOnContext(v -> {
      ...
      try {
            ...
            reply -> {
              try {
                if (reply.succeeded()) {
                  ...
                  Response response = Claim.PostClaimResponse.respond201WithApplicationJson(stream,
                      Claim.PostClaimResponse.headersFor201().withLocation(CLAIM_LOCATION_PREFIX + persistenceId));
                  respond(asyncResultHandler, response);
                } else {
                  log.error(reply.cause().getMessage(), reply.cause());
                  Response response = Claim.PostClaimResponse.respond500WithTextPlain(reply.cause().getMessage());
                  respond(asyncResultHandler, response);
                }
              } catch (Exception e) {
                log.error(e.getMessage(), e);

                Response response = Claim.PostClaimResponse.respond500WithTextPlain(e.getMessage());
                respond(asyncResultHandler, response);
              }
        ...
        Response response = Claim.PostClaimResponse.respond500WithTextPlain(errMsg);
        respond(asyncResultHandler, response);
      }
          ...
          postgresClient.delete(CLAIM_TABLE, id, reply -> {
            if (reply.succeeded()) {
              asyncResultHandler.handle(Future.succeededFuture(
                  Claim.DeleteClaimByIdResponse.noContent()
                    .build()));
            } else {
              asyncResultHandler.handle(Future.succeededFuture(
                  Claim.DeleteClaimByIdResponse.respond500WithTextPlain(reply.cause().getMessage())));
            }
          });
        } catch (Exception e) {
          asyncResultHandler.handle(Future.succeededFuture(
              Claim.DeleteClaimByIdResponse.respond500WithTextPlain(e.getMessage())));
        }
      });
    } catch (Exception e) {
      asyncResultHandler.handle(Future.succeededFuture(
