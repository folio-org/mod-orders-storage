  ...
  public void getOrdersStoragePoLineNumber(String purchaseOrderId, String lang, Map<String, String> okapiHeaders,
    ...
    vertxContext.runOnContext((Void v) -> {
      try {
        ...
        PostgresClient.getInstance(vertxContext.owner(), tenantId).select(GET_POL_NUMBER_FROM_SEQUENCE.getQuery(purchaseOrderId),
          getPolNumberReply -> {
          try {
            if (getPolNumberReply.succeeded()) {
              String poLineNumber = getPolNumberReply.result().getResults().get(0).getList().get(0).toString();
