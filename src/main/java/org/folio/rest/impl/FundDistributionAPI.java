...
public class FundDistributionAPI implements FundDistributionResource {
    ...
    public void getFundDistribution(String query, int offset, int limit, String lang, Map<String, String> okapiHeaders, Handler<AsyncResult<Response>> asyncResultHandler, Context vertxContext) throws Exception {
        vertxContext.runOnContext((Void v) -> {
            try {
                ...
                PostgresClient.getInstance(vertxContext.owner(), tenantId).get(FUND_DISTRIBUTION_TABLE, FundDistribution.class, fieldList, cql, true, false, reply -> {
                    try {
                        if (reply.succeeded()) {
                            FundDistributionCollection collection = new FundDistributionCollection();
                            @SuppressWarnings("unchecked")
                            List<FundDistribution> results = (List<FundDistribution>)reply.result().getResults();
    ...
    @Override
    public void postFundDistribution(String lang, FundDistribution entity, Map<String, String> okapiHeaders, Handler<AsyncResult<Response>> asyncResultHandler, Context vertxContext) throws Exception {
        vertxContext.runOnContext(v -> {
            ...
            try {
                        ...
                        reply -> {
                            try {
                                if (reply.succeeded()) {
                                    ...
                                    Response response = FundDistributionResource.PostFundDistributionResponse.
                                            withJsonCreated(FUND_DISTRIBUTION_LOCATION_PREFIX + persistenceId, stream);
                                    respond(asyncResultHandler, response);
                                }
                                else {
                                    log.error(reply.cause().getMessage(), reply.cause());
                                    Response response = FundDistributionResource.PostFundDistributionResponse.withPlainInternalServerError(reply.cause().getMessage());
                                    respond(asyncResultHandler, response);
                                }
                            }
                            catch (Exception e) {
                                ...
                                Response response = FundDistributionResource.PostFundDistributionResponse.withPlainInternalServerError(e.getMessage());
                                respond(asyncResultHandler, response);
                            }
                ...
                Response response = FundDistributionResource.PostFundDistributionResponse.withPlainInternalServerError(errMsg);
                respond(asyncResultHandler, response);
            }
    ...
    public void getFundDistributionById(String id, String lang, Map<String, String> okapiHeaders, Handler<AsyncResult<Response>> asyncResultHandler, Context vertxContext) throws Exception {
        vertxContext.runOnContext(v -> {
            try {
                ...
                PostgresClient.getInstance(vertxContext.owner(), tenantId).get(FUND_DISTRIBUTION_TABLE, FundDistribution.class, c, true,
                        reply -> {
                            try {
                                if (reply.succeeded()) {
                                    @SuppressWarnings("unchecked")
    ...
    @Override
    public void putFundDistributionById(String id, String lang, FundDistribution entity, Map<String, String> okapiHeaders, Handler<AsyncResult<Response>> asyncResultHandler, Context vertxContext) throws Exception {
        vertxContext.runOnContext(v -> {
            String tenantId = TenantTool.calculateTenantId( okapiHeaders.get(RestVerticle.OKAPI_HEADER_TENANT) );
            try {
                if(entity.getId() == null){
