         ...
         org.folio.rest.jaxrs.model.FundDistribution.class, fieldList, cql, true, false, reply -> {
           try {
             if (reply.succeeded()) {
               ...
               asyncResultHandler.handle(io.vertx.core.Future.succeededFuture(FundDistribution.GetFundDistributionResponse
  ...
  public void postFundDistribution(String lang, org.folio.rest.jaxrs.model.FundDistribution entity, Map<String, String> okapiHeaders, Handler<AsyncResult<Response>> asyncResultHandler, Context vertxContext) {
    vertxContext.runOnContext(v -> {
      ...
      try {
          ...
          reply -> {
            try {
              if (reply.succeeded()) {
                ...
                Response response = FundDistribution.PostFundDistributionResponse.respond201WithApplicationJson(stream,
                  FundDistribution.PostFundDistributionResponse.headersFor201().withLocation(FUND_DISTRIBUTION_LOCATION_PREFIX + persistenceId));
                respond(asyncResultHandler, response);
              } else {
                log.error(reply.cause().getMessage(), reply.cause());
                Response response = FundDistribution.PostFundDistributionResponse.respond500WithTextPlain(reply.cause().getMessage());
                respond(asyncResultHandler, response);
              }
            } catch (Exception e) {
              log.error(e.getMessage(), e);

              Response response = FundDistribution.PostFundDistributionResponse.respond500WithTextPlain(e.getMessage());
              respond(asyncResultHandler, response);
            }
        ...
        Response response = FundDistribution.PostFundDistributionResponse.respond500WithTextPlain(errMsg);
        respond(asyncResultHandler, response);
      }
          ...
          postgresClient.delete(FUND_DISTRIBUTION_TABLE, id, reply -> {
            if (reply.succeeded()) {
              asyncResultHandler.handle(Future.succeededFuture(
                FundDistribution.DeleteFundDistributionByIdResponse.noContent()
                  .build()));
            } else {
              asyncResultHandler.handle(Future.succeededFuture(
                FundDistribution.DeleteFundDistributionByIdResponse.respond500WithTextPlain(reply.cause().getMessage())));
            }
          });
        } catch (Exception e) {
          asyncResultHandler.handle(Future.succeededFuture(
            FundDistribution.DeleteFundDistributionByIdResponse.respond500WithTextPlain(e.getMessage())));
        }
      });
    } catch (Exception e) {
      asyncResultHandler.handle(Future.succeededFuture(
