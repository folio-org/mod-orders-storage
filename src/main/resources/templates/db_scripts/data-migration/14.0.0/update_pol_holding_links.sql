...
WITH po_lines_to_process AS (
    ...
    WHERE
        ...
        AND left(lower(${myuniversity}_${mymodule}.f_unaccent(po.jsonb->>'workflowStatus'::text)), 600) = 'open'
),
...
WHERE
    ...
    AND NOT (upd.current_locations_array @> upd.new_locations_array
             AND upd.current_locations_array <#noparse><@</#noparse> upd.new_locations_array);
