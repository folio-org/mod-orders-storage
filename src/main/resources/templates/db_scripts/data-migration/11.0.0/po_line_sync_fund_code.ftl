...
SET jsonb =
    (
      SELECT jsonb_set(jsonb, '{fundDistribution}', jsonb_agg(jsonb_set(distrib, '{code}', (SELECT jsonb -> 'code' FROM ${myuniversity}_mod_finance_storage.fund WHERE jsonb ->> 'id' = distrib ->> 'fundId'))))
